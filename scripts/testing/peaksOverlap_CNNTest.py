...
def main(_):
    ...
    model = tf.train.import_meta_graph('../../models/olapModel1.meta')
    ...
    y_conv = graph.get_tensor_by_name('add_5:0')
    y_hat = y_conv.eval(session=sess, feed_dict={x: test_seq[2500:2600, :], keep_prob: 0.3})

    # plot y_hat and y_valid
    plt.plot(np.arange(0, 100, 1), y_hat, 'r', np.arange(0, 100, 1), test_labels[2500:2600], 'b')
    plt.axis([0, 100, -2, 2])
    plt.savefig('olapPlot1.png')
    plt.show()

    error = graph.get_operation_by_name('l2_loss')
    error = error.values()[0]

    test_error = error.eval(session=sess, feed_dict={x: test_seq[2500:2600, :], y_: test_labels[2500:2600],
                                                                                                    keep_prob: 0.3})
