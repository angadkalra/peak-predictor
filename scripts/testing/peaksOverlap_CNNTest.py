...
def main(_):
    ...
    model = tf.train.import_meta_graph('../../models/olapModel2.meta')
    ...
    y_hat = y_conv.eval(session=sess, feed_dict={x: test_seq[:500, :], keep_prob: 0.3})

    # plot y_hat and y_valid
    plt.plot(y_hat, 'r', test_labels[:500], 'b')
    plt.show()

    error = graph.get_operation_by_name('l2_loss')
    error = error.values()[0]

    test_error = error.eval(session=sess, feed_dict={x: test_seq[:500], y_: test_labels[:500], keep_prob: 0.3})
